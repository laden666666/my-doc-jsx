<doc>
    ...
    <p>
    <strong>xml</strong>
    {`无疑是最优秀的标记语言，xml具有较高的可扩展性，定义规范。但是xml不适合书写文档，因为他过于`}<strong>规范</strong>{`，不够灵活，严格的语法要求使人生畏；而且他对于转义的处理过也不够方便，一个空格都需要六个字符表示（&nbsp;），也没几个人能够记住这些转义；同时离开编辑器我们也很难直接使用xml去书写复杂的格式文档。`}
    </p>
    <p>
...
这些问题都是因为markdown不具备xml的规范性导致的。`}
    ...
    <p>
    <strong>html</strong>
    {`本身就没有扩展性，同时有没有markdown方便，更不适合书写文档。包含了xml和markdown的全部缺点`}
    <strong>jsx</strong>
    {`对，你没看错，就是react提供的附属品——jsx。我们希望能够像写markdown一样写文档，又希望像写xml一样写文档代码，儿jsx不就是这样的一个标记语言吗？
同时配合es6的`}<strong>模板字符串</strong>{`，可以完美地解决了转义问题。
