...
那么用什么形式来作为这个标记语言呢？
    `}</p>
    <p>{`
`}<strong>xml</strong>{`无疑是最优秀的标记语言，xml具有较高的可扩展性，定义规范。但是xml不适合书写文档，因为他过于`}<strong>规范</strong>{`，不够灵活，严格的语法要求使人生畏；而且他对于转义的处理过也不够方便，一个空格都需要`}<strong>&nbsp;</strong>{`六个字符书才能写，而且也没几个人能够记住这些转义；同时离开编辑器我们也很难直接使用xml去书写复杂的格式文档。
    `}</p>
    <p>{`
`}<strong>markdown</strong>{`无疑是最流行的用于文档书写的标记语言，它非常的简单，而且就是为了书写文档而生的。
...
这些问题都是因为markdown不具备xml的规范性导致的。
    `}</p>
    <p>{`
`}<strong>html</strong>{`本身就没有扩展性，同时有没有markdown方便，更不适合书写文档。
